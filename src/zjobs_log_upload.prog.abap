*&---------------------------------------------------------------------*
*& Report ZUPLOAD_JOBS_LOG
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT zupload_jobs_log.

TYPES: BEGIN OF ty_log,
         job_id          TYPE zjobs_log-job_id,
         log_seq         TYPE zjobs_log-log_seq,
         action_taken    TYPE zjobs_log-action_taken,
         time_spent_hrs  TYPE zjobs_log-time_spent_hrs,
         technician_note TYPE zjobs_log-technician_note,
         attachment_url  TYPE zjobs_log-attachment_url,
         created_by      TYPE zjobs_log-created_by,
         created_on      TYPE zjobs_log-created_on,
       END OF ty_log.

DATA: lt_logs    TYPE TABLE OF ty_log,
      lv_file    TYPE string,
      lt_raw     TYPE STANDARD TABLE OF string,
      lv_line    TYPE string,
      lt_files   TYPE filetable,
      lv_rc      TYPE i,
      lv_length  TYPE i,
      lt_job_ids TYPE TABLE OF zjobs_log-job_id.

START-OF-SELECTION.

  " Select CSV file
  cl_gui_frontend_services=>file_open_dialog(
    EXPORTING
      file_filter = 'CSV Files (*.csv)|*.csv|'
    CHANGING
      file_table = lt_files
      rc = lv_rc
    EXCEPTIONS
      OTHERS = 1 ).

  IF lv_rc <= 0 OR lines( lt_files ) = 0.
    WRITE: / 'No file selected or operation canceled.'.
    RETURN.
  ENDIF.

  READ TABLE lt_files INDEX 1 INTO lv_file.

  " Upload file content to internal table
  cl_gui_frontend_services=>gui_upload(
    EXPORTING
      filename            = lv_file
      filetype            = 'ASC'
      has_field_separator = abap_true
    CHANGING
      data_tab = lt_raw
    EXCEPTIONS
      OTHERS = 1 ).

  IF sy-subrc <> 0.
    WRITE: / 'Error uploading file.'.
    RETURN.
  ENDIF.

  " Loop from line 2 to skip header
  LOOP AT lt_raw INTO lv_line FROM 2.

    DATA(ls_log) = VALUE ty_log( ).

    " Split CSV line fields
    DATA: lv_fields TYPE STANDARD TABLE OF string.
    SPLIT lv_line AT ',' INTO TABLE lv_fields.

    " Check if we have at least 8 fields (8 columns)
    IF lines( lv_fields ) >= 8.
      READ TABLE lv_fields INDEX 1 INTO ls_log-job_id.
      READ TABLE lv_fields INDEX 2 INTO DATA(lv_log_seq).
      READ TABLE lv_fields INDEX 3 INTO ls_log-action_taken.
      READ TABLE lv_fields INDEX 4 INTO DATA(lv_time_spent).
      READ TABLE lv_fields INDEX 5 INTO ls_log-technician_note.
      READ TABLE lv_fields INDEX 6 INTO ls_log-attachment_url.
      READ TABLE lv_fields INDEX 7 INTO ls_log-created_by.
      READ TABLE lv_fields INDEX 8 INTO DATA(lv_created_on).

      " Convert numeric and date fields
      ls_log-log_seq = lv_log_seq.

      " Convert decimal, replace ',' with '.' if any
      REPLACE ALL OCCURRENCES OF ',' IN lv_time_spent WITH '.'.
      ls_log-time_spent_hrs = lv_time_spent.

      " Convert date from DD-MM-YYYY to YYYYMMDD
      DATA(lv_date) = lv_created_on.
      SPLIT lv_date AT '-' INTO DATA(lv_dd) DATA(lv_mm) DATA(lv_yyyy).
      CONCATENATE lv_yyyy lv_mm lv_dd INTO ls_log-created_on.

      " Validation check for Attachment URL
      IF ls_log-attachment_url IS INITIAL.
        WRITE: / 'Warning: Attachment URL is missing for Job ID:', ls_log-job_id.
      ENDIF.

      " Append record to internal table
      APPEND ls_log TO lt_logs.
      APPEND ls_log-job_id TO lt_job_ids.
    ELSE.
      WRITE: / 'Line skipped due to incorrect format: ', lv_line.
    ENDIF.

  ENDLOOP.

  " Remove duplicates from the list of JOB_IDs
  DELETE ADJACENT DUPLICATES FROM lt_job_ids.

  " Delete only the relevant JOB_IDs from the database
  LOOP AT lt_job_ids INTO DATA(lv_job_id).
    DELETE FROM zjobs_log WHERE job_id = lv_job_id.
  ENDLOOP.

  " Insert all data to database table
  INSERT zjobs_log FROM TABLE lt_logs.

  IF sy-subrc = 0.
    COMMIT WORK.
    WRITE: / 'Data successfully inserted:', lines( lt_logs ).
  ELSE.
    WRITE: / 'Data insertion failed.'.
  ENDIF.
